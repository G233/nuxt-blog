...
class User {
  ...
  static async checkcode(ctx) {
    let { code, email } = ctx.request.body;
  ...
  static async register(ctx) {
    ...
    let { name, email, passwd } = ctx.request.body;
  ...
  //   let A = await ArticleModel.create({ titel: ctx.request.body.name, author: userid })
  //   let U = await UserModel.findOne({ _id: userid })
  //   console.log(U)
  //   U.articles.push(A)
  //   U.save()

  //   let y = await ArticleModel.findOne({ titel: ctx.request.body.name })
